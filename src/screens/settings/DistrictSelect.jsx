...
export default function DistrictSelect() {
  ...
  const data = useMemo(() => {
    const list = Array.isArray(districts) ? districts.filter((d) => String(d.sehir_id) === cityId) : [];
    if (!q) return list;
    const t = q.toLowerCase();
    return list.filter((d) => String(d.ilce_adi).toLowerCase().includes(t));
  }, [q, cityId]);
    ...
    <TouchableOpacity style={styles.itemRow} onPress={() => { onSelect?.(item); navigation.goBack(); }}>
      <View style={styles.itemLeft}>
        ...
        <View style={{ marginLeft: s(10), flex: 1 }}>
          <Text style={styles.itemTitle}>{item.ilce_adi}</Text>
          <Text style={styles.itemSubtitle}>İlçe</Text>
        </View>
      </View>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.white }}>
      ...
      <View style={{ paddingHorizontal: s(16) }}>
        <TextInput value={q} onChangeText={setQ} style={styles.search} placeholder="İlçe ara" placeholderTextColor={Colors.gray} />
      </View>
      <FlatList
        data={data}
        keyExtractor={(x) => String(x.ilce_id)}
